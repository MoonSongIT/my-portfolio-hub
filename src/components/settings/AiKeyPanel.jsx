...
import { Eye, EyeOff, CheckCircle2, XCircle, Loader2, AlertCircle, ExternalLink } from 'lucide-react'
...
export default function AiKeyPanel() {
  const { apiKey, isValid, validatedAt, isLoading, hasKey, setApiKey, validateKey, clearKey } = useAiCredentialStore()
  ...
  function StatusBadge() {
    if (!keyExists) {
      return <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">미설정</span>
    }
    if (isValid === true) {
      return (
        <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400">
          ...
          검증 완료
          {validatedAt && <span className="text-green-600/70 dark:text-green-500/70">({new Date(validatedAt).toLocaleDateString('ko-KR')})</span>}
        </span>
      )
    }
    if (isValid === false) {
    ...
    return <span className="text-xs px-2 py-0.5 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400">미검증</span>
  }
  ...
  async function handleValidate() {
    ...
    if (ok) {
      toast.success('API 키 검증 성공!')
    } else {
      toast.error('API 키 검증 실패. 키를 확인해주세요.')
    }
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="font-medium text-gray-900 dark:text-white">Anthropic API 키</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">AI 기능 사용에 필요합니다.</p>
        </div>
        <StatusBadge />
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 text-sm font-medium disabled:opacity-40 transition"
        >
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 max-w-sm w-full space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-white">API 키 삭제</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400">AI 기능을 사용할 수 없게 됩니다</p>
              </div>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-300">저장된 API 키를 삭제하시겠습니까?</p>
