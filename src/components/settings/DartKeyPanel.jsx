...
export default function DartKeyPanel() {
  const { dartApiKey, dartSavedAt, hasDartKey, setDartKey, clearDartKey } = useAiCredentialStore()
  const [inputValue, setInputValue] = useState('')
  const [showKey, setShowKey] = useState(false)
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false)

  const keyExists = hasDartKey()
      ...
      <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400">
        ...
        {dartSavedAt && (
          <span className="text-green-600/70 dark:text-green-500/70">
            ({new Date(dartSavedAt).toLocaleDateString('ko-KR')})
          </span>
        )}
      </span>
    )
  ...
  async function handleSave() {
    ...
    await setDartKey(key)
    setInputValue('')
    toast.success('DART API 키가 저장되었습니다.')
  }

  async function handleDelete() {
    await clearDartKey()
    setDeleteConfirmOpen(false)
    toast.success('DART API 키가 삭제되었습니다.')
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="font-medium text-gray-900 dark:text-white">DART API 키</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            국내 주식 공시 조회 및 종목 마스터 DB 수집에 사용됩니다.
          </p>
        </div>
        <StatusBadge />
      </div>
      ...
      {keyExists && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm font-mono text-gray-600 dark:text-gray-400">
          {showKey ? dartApiKey : maskKey(dartApiKey)}
          <button
      ...
      <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 text-xs">
        ...
        <span>
          ...
          <a
            href="https://opendart.fss.or.kr/uat/uia/easyLogin.do"
            target="_blank"
            rel="noopener noreferrer"
            className="underline inline-flex items-center gap-0.5"
          >
            DART 키 발급 <ExternalLink className="w-3 h-3" />
          </a>
        </span>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 max-w-sm w-full space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-white">DART API 키 삭제</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400">공시 조회 기능을 사용할 수 없게 됩니다</p>
              </div>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-300">저장된 DART API 키를 삭제하시겠습니까?</p>
