...
// 키 유효성 검증 핸들러 — POST /api/claude/validate
async function handleValidate(req, res) {
  ...
  try {
    ...
    const err = await response.text()
    return res.status(200).json({ valid: false, reason: `API 오류 (${response.status}): ${err.substring(0, 100)}` })
  } catch (err) {
